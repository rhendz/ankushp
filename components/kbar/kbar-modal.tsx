          ...
          <KBarAnimator className="w-full max-w-xl">
            <div className="overflow-hidden rounded-2xl border border-secondary/20 bg-primary/70">
              <div className="flex items-center space-x-4 p-4">
                ...
                <KBarSearch className="h-8 w-full bg-transparent text-secondary placeholder-secondary-50 focus:outline-none" />
                <kbd className="inline-block whitespace-nowrap rounded border px-1.5 align-middle font-medium leading-4 tracking-wide text-xs text-secondary/70 border-secondary/70">
                  ESC
                </kbd>
              </div>
              ...
              {typeof item === 'string' ? (
                ...
                <div
                  className={`flex cursor-pointer justify-between px-4 py-2 ${
                    active
                      ? 'bg-accent/70 text-secondary'
                      : 'text-secondary/50 bg-transparent'
                  }`}
                >
                  <div className={'flex space-x-2'}>
                    ...
                    <div className="block">
                      {item.subtitle && (
                        <div className={`${active ? 'text-secondary/70' : 'text-secondary/50'} text-xs`}>
                          {item.subtitle}
                        </div>
                      )}
                      <div>{item.name}</div>
                      ...
                      {item.shortcut.map((sc) => (
                        <kbd
                          ...
                          className={`font-medium h-7 w-6 flex items-center	justify-center text-xs rounded border ${
                            active ? 'text-secondary/30 border-secondary/30' : 'text-secondary/50 border-secondary/50'
                          }`}
                        >
                          {sc}
                        </kbd>
