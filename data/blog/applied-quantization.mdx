...
<div className="flex w-full items-center justify-center">
  <Diagram src="./applied-quantization/diagram00" />
</div>
...
So what does this process actually look like? Well let's start with some random weights:

<div className="flex h-[200px] items-center justify-center">
  <Diagram src="./applied-quantization/diagram01" />
</div>

We can quantize these weights to 8-bit by mapping them to the range $$[-127,127]$$. Although we won't cover the mapping process here, the general idea is to find a scale factor that we can multiply with our floating point numbers to get an integer value bounded by the range mentioned previously. Note, we don't use $$128$$ for the sake of symmetry.

<div className="flex h-[200px] items-center justify-center">
  <Diagram src="./applied-quantization/diagram02" />
</div>
...
Finally, we can dequantize the weights. As you can see, the dequantized weights are quite similar to the original weights with a mean-squared error of $6.8\%$.

<div className="flex h-[200px] items-center justify-center">
  <Diagram src="./applied-quantization/diagram03" />
</div>
