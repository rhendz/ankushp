...
**Quantization** is a method used to approximate a neural network in some high precision format using floating-point (e.g. fp32) to some low bit width format (e.g. int8).
One such example is shown below, where we _map_ the 32-bit floating point range to an 8-bit integer range [-128,127].
...
Real values in the continuous domain $$r$$ are mapped into discrete, low-precision values in the
...
With a mapping technique in place, we can fine-tune existing models:

- Post-Training Quantization (PTQ) as its name implies is performed after training some model. It is efficient in situations where data is limited or unlabeled.
