...
Our first technique is _pruning_. **Pruning** is a technique used to reduce the size of a neural network model by identifying
...
There are three dimensions that describe aspects of pruning: form, scope, and method.
...
architecture. On the other hand, **unstructured pruning** removes individual weights/synapses, resulting in a sparse weights.

Although unstructured pruning may achieve higher compression rates, it can be more challenging to implement efficiently, especially on hardware without native support for irregularly sparse tensors (matrices with lots of random 0's).

### Scope: Global vs. Local

The _scope_ refers to the extent to which pruning is applied within the model. **Global pruning** is pruning across the entire neural network architecture, whereas **local pruning** as the name suggests is pruning over specific regions or parts of the architecture.
...
- Better for hardware optimization
...
- Fine-grained control over model sparsity
- Potential for performance preservation
- Flexibility in implementation

_Cons:_

- Limited compression rates
- Increase computational complexity
- Risk of overfitting

Global pruning can achieve significant model compression by removing redundant or less important structural components, however, it may lead to over-pruning or loss of important information if not carefully controlled. On the other hand, local pruning offers fine-grained control over what we
want to prune, leading to the pruning of some componenents and the preservation of others. This can be important if we have sensitive regions of the model, where we must preserve important representations. Note, local pruning has limited compression and is more prone to overfitting when compared to
global pruning.

Hmmm... perhaps it would be better to discuss the pros and cons of each method in a later post and instead focus on the bigger picture and definitions. Talk about pruning in general!

### Method: Iterative vs. One-Shot
